...
<main class="min-h-screen bg-[#fbfcfa] text-[#102219]">
	<header class="sticky top-0 z-40 border-b border-black/[0.06] bg-[#fbfcfa]/90 px-4 backdrop-blur-xl sm:px-6 lg:px-10">
		<nav class="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4" aria-label="Navigasi utama">
			<a class="flex min-w-0 items-center gap-3" href="/" aria-label="SantriOnline beranda">
				<img src={logo} alt="Logo SantriOnline" class="size-10 rounded-xl border border-black/[0.06] object-cover shadow-sm" />
				<div class="min-w-0">
					<p class="truncate text-base font-extrabold tracking-[-0.02em] sm:text-lg">SantriOnline</p>
					<p class="hidden text-[10px] font-bold uppercase tracking-[0.16em] text-[#718078] sm:block">Tumbuh · Beradab · Berdampak</p>
				</div>
			</a>

			<div class="hidden items-center gap-7 text-sm font-semibold text-[#52635a] md:flex">
				{#each navigation as item}
					<a class="transition hover:text-[#11633c]" href={item.href}>{item.label}</a>
				{/each}
			</div>

			<div class="hidden items-center gap-2 md:flex">
				<a class="rounded-full border border-black/10 bg-white px-4 py-2 text-sm font-bold transition hover:border-[#16734a]/40 hover:text-[#11633c]" href={appLoginUrl}>Masuk</a>
				<a class="rounded-full bg-[#12663e] px-5 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-[#0d5030] focus:outline-none focus:ring-4 focus:ring-[#12663e]/15" href={appBaseUrl}>Buka Aplikasi <span aria-hidden="true">↗</span></a>
			</div>
	...
	<section class="relative px-4 pb-16 pt-14 sm:px-6 sm:pt-20 lg:px-10 lg:pb-24 lg:pt-24">
		<div class="pointer-events-none absolute inset-x-0 top-0 -z-0 h-[720px] bg-[radial-gradient(circle_at_16%_15%,rgba(51,169,105,0.16),transparent_32%),radial-gradient(circle_at_85%_30%,rgba(211,171,71,0.14),transparent_27%)]"></div>
		<div class="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.04fr_0.96fr] lg:items-center">
			<div class="max-w-3xl">
				<p class="inline-flex items-center gap-2 rounded-full border border-[#12663e]/15 bg-white/80 px-4 py-2 text-xs font-bold uppercase tracking-[0.12em] text-[#12663e] shadow-sm">
					<span class="size-2 rounded-full bg-[#31a86b]"></span>
					Ekosistem Pembinaan Generasi Muslim
				</p>
				<h1 class="mt-7 text-[2.8rem] font-extrabold leading-[1.04] tracking-[-0.055em] text-[#0d2116] sm:text-6xl lg:text-[4.6rem]">
				...
				<div class="mt-9 flex flex-col gap-3 sm:flex-row">
					<a class="inline-flex items-center justify-center gap-2 rounded-full bg-[#12663e] px-7 py-3.5 text-base font-bold text-white shadow-[0_10px_30px_rgba(18,102,62,0.18)] transition hover:-translate-y-0.5 hover:bg-[#0d5030]" href={appRegisterUrl}>
						Mulai Perjalanan Santri <span aria-hidden="true">→</span>
					</a>
					<a class="inline-flex items-center justify-center rounded-full border border-black/10 bg-white px-7 py-3.5 text-base font-bold text-[#263d31] shadow-sm transition hover:border-[#12663e]/40 hover:text-[#12663e]" href="#ekosistem">
						Lihat Ekosistem
					</a>
				</div>
			...
			<div class="relative lg:pl-4">
				<div class="absolute -inset-5 rounded-[40px] bg-gradient-to-br from-[#36a86d]/15 to-[#d7b557]/15 blur-2xl"></div>
				<div class="relative overflow-hidden rounded-[28px] border border-black/[0.07] bg-white shadow-[0_30px_80px_rgba(23,70,45,0.14)]">
					<div class="flex items-center justify-between border-b border-black/[0.06] px-5 py-4">
						...
						<span class="rounded-full bg-[#e4f6eb] px-3 py-1 text-[11px] font-bold text-[#12663e]">TERHUBUNG</span>
					</div>

					<div class="bg-gradient-to-b from-[#f4faf6] to-white p-5 sm:p-7">
						<div class="rounded-2xl bg-[#102a1d] p-5 text-white">
							<p class="text-xs font-bold uppercase tracking-[0.14em] text-[#9ed8b9]">Fokus Hari Ini</p>
							<div class="mt-4 flex items-end justify-between gap-4">
								...
								<span class="grid size-12 shrink-0 place-items-center rounded-full bg-[#d8b654] text-xl text-[#102a1d]">↗</span>
							</div>
						</div>

						<div class="mt-4 grid grid-cols-2 gap-3">
							<a class="rounded-2xl border border-black/[0.06] bg-white p-4 transition hover:-translate-y-0.5 hover:border-[#12663e]/30 hover:shadow-md" href={`${appBaseUrl}/belajar`}>
								<span class="grid size-9 place-items-center rounded-xl bg-[#e5f5eb] text-sm font-black text-[#12663e]">IL</span>
								<p class="mt-4 text-sm font-extrabold">Lanjut Belajar</p>
								<p class="mt-1 text-xs leading-5 text-[#718078]">Materi bertahap</p>
							</a>
							<a class="rounded-2xl border border-black/[0.06] bg-white p-4 transition hover:-translate-y-0.5 hover:border-[#c29832]/30 hover:shadow-md" href={`${appBaseUrl}/kitab`}>
								<span class="grid size-9 place-items-center rounded-xl bg-[#fbf1d5] text-sm font-black text-[#8a6820]">KT</span>
							...
							<a class="rounded-2xl border border-black/[0.06] bg-white p-4 transition hover:-translate-y-0.5 hover:border-[#c29832]/30 hover:shadow-md" href={`${appBaseUrl}/dashboard`}>
								<span class="grid size-9 place-items-center rounded-xl bg-[#fbf1d5] text-sm font-black text-[#8a6820]">PR</span>
								<p class="mt-4 text-sm font-extrabold">Lihat Progres</p>
								<p class="mt-1 text-xs leading-5 text-[#718078]">Pantau pertumbuhan</p>
							</a>
						</div>
					</div>

					<a class="flex items-center justify-between border-t border-black/[0.06] px-5 py-4 text-sm font-bold text-[#12663e] transition hover:bg-[#f4faf6]" href={appBaseUrl}>
	...
	<section id="arah" class="scroll-mt-20 border-y border-black/[0.05] bg-white px-4 py-16 sm:px-6 lg:px-10 lg:py-24">
		<div class="mx-auto max-w-7xl">
			<div class="grid gap-10 lg:grid-cols-[0.78fr_1.22fr] lg:items-end">
				<div>
					<p class="text-xs font-bold uppercase tracking-[0.18em] text-[#a87f25]">Arah SantriOnline</p>
					<h2 class="mt-4 text-3xl font-extrabold leading-tight tracking-[-0.04em] sm:text-5xl">Bukan sekadar konten. Ini perjalanan pembentukan diri.</h2>
				</div>
				<p class="max-w-2xl text-lg leading-8 text-[#5d6d64] lg:justify-self-end">
			...
			<div class="mt-12 grid gap-px overflow-hidden rounded-3xl border border-black/[0.06] bg-black/[0.06] md:grid-cols-3">
				<div class="bg-[#fbfcfa] p-7 sm:p-8">
					<p class="text-xs font-bold uppercase tracking-[0.15em] text-[#12663e]">01 · Identitas</p>
					<h3 class="mt-5 text-xl font-extrabold">Aku muslim yang punya misi</h3>
					<p class="mt-3 leading-7 text-[#607067]">Aqidah, adab, dan teladan Rasulullah ﷺ menjadi fondasi sebelum mengejar prestasi.</p>
				</div>
				<div class="bg-[#fbfcfa] p-7 sm:p-8">
					<p class="text-xs font-bold uppercase tracking-[0.15em] text-[#12663e]">02 · Kebiasaan</p>
					<h3 class="mt-5 text-xl font-extrabold">Ilmu berubah menjadi amal</h3>
					<p class="mt-3 leading-7 text-[#607067]">Misi harian dan progres membantu langkah kecil tumbuh menjadi istiqamah.</p>
				</div>
				<div class="bg-[#fbfcfa] p-7 sm:p-8">
	...
	<section id="ekosistem" class="scroll-mt-20 px-4 py-16 sm:px-6 lg:px-10 lg:py-24">
		<div class="mx-auto max-w-7xl">
			...
			<div class="mt-12 grid gap-6 lg:grid-cols-[0.88fr_auto_1.12fr] lg:items-stretch">
				<article class="rounded-3xl border border-black/[0.06] bg-white p-7 shadow-sm sm:p-9">
					<span class="inline-flex rounded-full bg-[#eef5f0] px-3 py-1 text-xs font-bold uppercase tracking-[0.12em] text-[#4e6458]">santrionline.com</span>
					<h3 class="mt-6 text-2xl font-extrabold tracking-[-0.03em]">Pintu publik</h3>
					<p class="mt-3 leading-7 text-[#607067]">Mengenal visi, arah pembinaan, jalur belajar, dan manfaat SantriOnline.</p>
					<ul class="mt-6 space-y-3 text-sm font-semibold text-[#40564a]">
						<li class="flex gap-3"><span class="text-[#20925b]">✓</span> Memahami misi SantriOnline</li>
						<li class="flex gap-3"><span class="text-[#20925b]">✓</span> Mencoba asisten AI publik</li>
						<li class="flex gap-3"><span class="text-[#20925b]">✓</span> Memilih jalur yang sesuai</li>
					</ul>
				</article>

				<div class="hidden items-center justify-center lg:flex" aria-hidden="true">
					<span class="grid size-14 place-items-center rounded-full bg-[#12663e] text-xl text-white shadow-lg">→</span>
				</div>

				<article class="relative overflow-hidden rounded-3xl bg-[#102a1d] p-7 text-white shadow-[0_24px_60px_rgba(16,42,29,0.18)] sm:p-9">
					<div class="absolute right-0 top-0 size-48 rounded-full bg-[#2aa466]/20 blur-3xl"></div>
					<span class="relative inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-bold uppercase tracking-[0.12em] text-[#a9ddbf]">app.santrionline.com</span>
			...
			<div class="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
				{#each appShortcuts as shortcut}
					<a class="group rounded-2xl border border-black/[0.06] bg-white p-5 transition hover:-translate-y-1 hover:border-[#12663e]/25 hover:shadow-lg" href={shortcut.href}>
						<div class="flex items-center justify-between">
							<span class={`grid size-10 place-items-center rounded-xl text-xs font-black ${shortcut.accent === 'gold' ? 'bg-[#fbf1d5] text-[#87671f]' : 'bg-[#e5f5eb] text-[#12663e]'}`}>{shortcut.marker}</span>
							<span class="text-[#849188] transition group-hover:translate-x-1 group-hover:text-[#12663e]">→</span>
						</div>
						<h3 class="mt-5 font-extrabold">{shortcut.label}</h3>
						<p class="mt-2 text-sm leading-6 text-[#65746c]">{shortcut.description}</p>
					</a>
				{/each}
			</div>
		</div>
			...
			<div class="grid gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:items-start">
				...
				<div class="grid gap-4 sm:grid-cols-2">
					{#each pillars as pillar}
						<article class="rounded-2xl border border-black/[0.06] bg-[#fbfcfa] p-6 transition hover:border-[#12663e]/20 hover:bg-[#f6faf7]">
							<span class="grid size-11 place-items-center rounded-xl bg-[#e5f5eb] text-xs font-black text-[#12663e]">{pillar.marker}</span>
							<h3 class="mt-5 text-xl font-extrabold tracking-[-0.02em]">{pillar.title}</h3>
							<p class="mt-3 leading-7 text-[#607067]">{pillar.description}</p>
						</article>
					{/each}
				</div>
			</div>
	...
	<section class="px-4 py-16 sm:px-6 lg:px-10 lg:py-24">
		<div class="mx-auto max-w-7xl">
			<div class="max-w-3xl">
				<p class="text-xs font-bold uppercase tracking-[0.18em] text-[#a87f25]">Tumbuh Bersama</p>
				<h2 class="mt-4 text-3xl font-extrabold tracking-[-0.04em] sm:text-5xl">Satu ekosistem untuk santri, pendamping, dan lembaga.</h2>
			</div>
			<div class="mt-12 grid gap-5 lg:grid-cols-3">
				{#each audiences as audience, index}
					<article class={`rounded-3xl p-7 sm:p-8 ${index === 1 ? 'bg-[#102a1d] text-white' : 'border border-black/[0.06] bg-white'}`}>
						<p class={`text-xs font-bold uppercase tracking-[0.15em] ${index === 1 ? 'text-[#a9ddbf]' : 'text-[#12663e]'}`}>0{index + 1}</p>
						<h3 class="mt-6 text-2xl font-extrabold tracking-[-0.03em]">{audience.name}</h3>
						<p class={`mt-3 min-h-24 leading-7 ${index === 1 ? 'text-white/70' : 'text-[#607067]'}`}>{audience.description}</p>
						<div class="mt-6 flex flex-wrap gap-2">
							{#each audience.links as link, linkIndex}
								<a class={`rounded-full px-4 py-2 text-sm font-bold transition ${index === 1 ? (linkIndex === 0 ? 'bg-white text-[#102a1d]' : 'border border-white/20 text-white hover:bg-white/10') : (linkIndex === 0 ? 'bg-[#12663e] text-white' : 'border border-black/10 text-[#40564a] hover:border-[#12663e]/30')}`} href={link.href}>{link.label}</a>
							{/each}
						</div>
					</article>
				{/each}
	...
	<section id="tanya" class="scroll-mt-20 bg-[#eef6f1] px-4 py-16 sm:px-6 lg:px-10 lg:py-24">
		<div class="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:items-center">
			<div>
				...
				<h2 class="mt-4 text-3xl font-extrabold tracking-[-0.04em] sm:text-5xl">Mulai dari satu pertanyaan yang baik.</h2>
				<p class="mt-5 text-lg leading-8 text-[#5d6d64]">Gunakan asisten publik untuk orientasi awal. Untuk pengalaman lengkap dan riwayat percakapan, lanjutkan di aplikasi.</p>
				<div class="mt-7 flex flex-wrap gap-2">
					{#each examples as example}
						<button type="button" class="rounded-full border border-[#12663e]/15 bg-white px-4 py-2 text-left text-xs font-semibold leading-5 text-[#40564a] transition hover:border-[#12663e]/40 hover:text-[#12663e]" disabled={isChatLoading} onclick={() => void sendQuestion(example)}>{example}</button>
					{/each}
				</div>
			</div>
			...
			<div class="overflow-hidden rounded-[28px] border border-black/[0.07] bg-white shadow-[0_24px_70px_rgba(23,70,45,0.12)]" role="region" aria-label="Chat demo SantriOnline">
				<div class="flex items-center justify-between border-b border-black/[0.06] px-5 py-4">
					<div class="flex items-center gap-3">
						<img src={logo} alt="" class="size-10 rounded-xl object-cover" />
						<div><p class="text-sm font-extrabold">SantriOnline AI</p><p class="text-xs text-[#718078]">Asisten orientasi belajar</p></div>
					</div>
					<a class="rounded-full bg-[#f8efd5] px-3 py-1.5 text-xs font-bold text-[#80621f]" href={groupWaUrl}>Komunitas</a>
				</div>

				<div class="min-h-[290px] p-5 sm:p-7" aria-live="polite">
					{#if chatMessages.length === 0}
						<div class="grid min-h-[235px] place-items-center text-center">
							<div><p class="text-2xl font-extrabold tracking-[-0.03em] sm:text-3xl">Apa yang ingin Anda tanyakan?</p><p class="mx-auto mt-3 max-w-md leading-7 text-[#65746c]">Tanyakan tentang adab, jalur belajar, kebiasaan baik, atau cara memulai sebagai santri digital.</p></div>
						</div>
					{:else}
						<div class="space-y-4">
							{#each chatMessages as message}
								<div class={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
									<div class={`max-w-[88%] whitespace-pre-line rounded-2xl px-4 py-3 text-sm leading-7 ${message.role === 'user' ? 'rounded-br-md bg-[#12663e] text-white' : 'rounded-bl-md bg-[#eef6f1] text-[#40564a]'}`}>{message.content}</div>
								</div>
							{/each}
							{#if isChatLoading}<p class="text-sm text-[#718078]">SantriOnline sedang menjawab…</p>{/if}
						</div>
					{/if}
				</div>

				<div class="border-t border-black/[0.06] p-4 sm:p-5">
					<form class="flex items-center gap-2 rounded-full border border-black/10 bg-[#f7f9f7] p-2 focus-within:border-[#12663e]/50 focus-within:ring-4 focus-within:ring-[#12663e]/10" onsubmit={handleSubmit}>
						<label class="sr-only" for="question">Ketik pertanyaan</label>
						<input id="question" class="min-w-0 flex-1 bg-transparent px-3 py-2 text-base outline-none placeholder:text-[#8b978f]" bind:value={question} placeholder="Ketik pertanyaan…" autocomplete="off" />
						<button type="submit" disabled={isChatLoading || !question.trim()} class="grid size-11 shrink-0 place-items-center rounded-full bg-[#12663e] text-white transition hover:bg-[#0d5030] disabled:cursor-not-allowed disabled:bg-[#cbd6cf]" aria-label="Kirim pertanyaan">↑</button>
					</form>
					<p class="mt-3 text-center text-[11px] leading-5 text-[#78857d]">Bukan pengganti ustadz atau mufti. Jawaban agama tetap perlu rujukan kitab dan guru terpercaya.</p>
				</div>
			</div>
		</div>
	</section>
	...
	<section class="px-4 py-16 sm:px-6 lg:px-10 lg:py-24">
		<div class="relative mx-auto max-w-7xl overflow-hidden rounded-[32px] bg-[#102a1d] px-6 py-12 text-white sm:px-10 lg:px-14 lg:py-16">
			<div class="absolute -right-20 -top-20 size-72 rounded-full bg-[#2aa466]/25 blur-3xl"></div>
			<div class="absolute -bottom-28 left-1/3 size-64 rounded-full bg-[#d4ad49]/15 blur-3xl"></div>
			<div class="relative grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
				<div class="max-w-3xl">
					<p class="text-xs font-bold uppercase tracking-[0.18em] text-[#e1c56a]">Mulai Hari Ini</p>
					<h2 class="mt-4 text-3xl font-extrabold tracking-[-0.04em] sm:text-5xl">Jadilah santri digital yang punya arah, adab, dan misi.</h2>
				...
				<div class="flex flex-col gap-3 sm:flex-row lg:flex-col">
					<a class="inline-flex items-center justify-center gap-2 rounded-full bg-white px-7 py-3.5 text-base font-bold text-[#102a1d]" href={appRegisterUrl}>Daftar Sekarang <span>→</span></a>
...
<footer class="border-t border-black/[0.06] bg-white px-4 py-10 text-[#5d6d64] sm:px-6 lg:px-10">
	<div class="mx-auto grid max-w-7xl gap-8 md:grid-cols-[1fr_auto] md:items-end">
		...
		<div class="md:text-right">
			<nav class="flex flex-wrap gap-x-5 gap-y-3 text-sm font-semibold md:justify-end" aria-label="Navigasi footer">
				<a class="hover:text-[#12663e]" href="#arah">Arah</a>
...
<nav class="fixed inset-x-0 bottom-0 z-40 border-t border-black/[0.07] bg-white/95 p-2 shadow-[0_-12px_40px_rgba(16,42,29,0.10)] backdrop-blur-xl md:hidden" aria-label="Akses cepat aplikasi">
	<div class="mx-auto grid max-w-md grid-cols-[1fr_1.25fr_1fr] gap-2">
		<a class="grid place-items-center rounded-2xl px-3 py-2 text-[11px] font-bold text-[#53665b]" href={appLoginUrl}><span class="text-base">◎</span><span>Masuk</span></a>
		<a class="grid place-items-center rounded-2xl bg-[#12663e] px-3 py-2 text-[11px] font-bold text-white shadow-md" href={appBaseUrl}><span class="text-base">↗</span><span>Buka Aplikasi</span></a>
		<a class="grid place-items-center rounded-2xl px-3 py-2 text-[11px] font-bold text-[#53665b]" href={`${appBaseUrl}/belajar`}><span class="text-base">◫</span><span>Belajar</span></a>
	</div>
</nav>
	...
	<div class="fixed inset-0 z-50 grid place-items-center bg-[#102a1d]/65 px-5 backdrop-blur-sm" role="presentation">
		<div class="w-full max-w-md rounded-[28px] bg-white p-6 text-[#102219] shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="upgrade-title">
			...
			<div class="mt-6 grid grid-cols-2 gap-3">
				<a class="rounded-full bg-[#12663e] px-5 py-3 text-center text-sm font-bold text-white" href={appRegisterUrl}>Daftar</a>
				<a class="rounded-full border border-black/10 px-5 py-3 text-center text-sm font-bold" href={appLoginUrl}>Masuk</a>
			</div>
		</div>
	</div>
{/if}
