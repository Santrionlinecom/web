<script lang="ts">
	// src/routes/fitur/[slug]/+page.svelte — satu halaman per kata kunci lembaga.
	// Isi dari $lib/fitur-lembaga.ts. Pola visual mengikuti /lembaga.
	import logo from '$lib/assets/logo.png';
	import type { PageData } from './$types';

	let { data }: { data: PageData } = $props();
	const f = $derived(data.fitur);
	const terkait = $derived(data.terkait);

	const appUrl = 'https://app.santrionline.com';
	const daftarUrl = `${appUrl}/lembaga/tambah`;
	const canonicalUrl = $derived(`https://santrionline.com/fitur/${f.slug}`);
	...
	<header class="sticky top-0 z-40 border-b border-so-border/80 bg-so-cream/90 backdrop-blur-xl">
		<div class="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
			...
			<div class="flex items-center gap-2">
				...
				<a href={waPendampingan} class="inline-flex items-center gap-2 rounded-full bg-so-green px-4 py-2.5 text-sm font-bold text-white hover:bg-so-green-3" rel="noopener">WhatsApp</a>
			</div>
		</div>
	</header>
	...
	<main>
		...
		{#if terkait.length}
			<section class="border-t border-so-border/70 bg-white px-4 py-12 sm:px-6" aria-labelledby="terkait-title">
				<div class="mx-auto max-w-6xl">
					<h2 id="terkait-title" class="text-xs font-bold uppercase tracking-[0.18em] text-so-accent-ink">Fitur lembaga lainnya</h2>
					<div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
						{#each terkait as t}
							<a href={`/fitur/${t.slug}`} class="rounded-2xl border border-so-border bg-so-cream p-5 hover:border-so-green/40">
